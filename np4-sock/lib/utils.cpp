...
int getClientSocket(struct sockaddr_in &cliAddr, const char *hostname, int port, bool nonblocking) {
    int sockfd;
    ...
    cliAddr.sin_addr = *((struct in_addr *)hostentry->h_addr_list[0]);
    if (nonblocking) {
...
int getListenSocket(const int port, const int listenQ) {
    ...
    return getListenSocketWithAddr(port, servAddr, listenQ);
}

int getListenSocketWithAddr(const int port, struct sockaddr_in &servAddr, const int listenQ) {
    int sockfd;
