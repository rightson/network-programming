...
public:
    ...
    void showStatus400(bool also_print_body = true) {
        utils::writeFD(connfd, "HTTP/1.1 400 Bad Request\r\n");
        if (also_print_body) {
            utils::writeFD(connfd, "\r\n<h1>400 Bad Request</h1>\r\n");
        }
    }
    void showStatus401(bool also_print_body = true) {
        utils::writeFD(connfd, "HTTP/1.1 401 Unauthorized\r\n");
        if (also_print_body) {
            utils::writeFD(connfd, "\r\n<h1>401 Unauthorized</h1>\r\n");
        }
    }
    void showStatus404(bool also_print_body = true) {
    ...
    int serveStaticFile(bool useCStyleFileIO = false) {
        if (utils::is_directory(script_filename.c_str())) {
            showStatus404();
            return -1;
        }
        printf("[server] serve static file [%s]\n", script_filename.c_str());
        if (useCStyleFileIO) {
